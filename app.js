...
// Define a cron job to run daily at midnight (00:00)
cron.schedule('0 0 * * *', () => {
  // Reset room statuses for the next day
...
app.post('/reset-room-status', (req, res) => {
    ...
    con.query('SELECT * FROM users WHERE user_id = ?', [userId], (error, results) => {
        ...
        // No need to manually reset room statuses here since it's handled by the cron job
        res.redirect('/staff/room-lists');
    });
});
